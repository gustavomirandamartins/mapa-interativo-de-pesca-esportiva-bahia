...
## Correção de zoom e modo off-line de evento

- **Fix: "Map data not yet available" ao aproximar o zoom.** O World_Ocean_Base do Esri só tem imagem real até o zoom 10 nesta região — zoom 11+ sempre devolvia o mesmo tile-placeholder do próprio Esri, com HTTP 200 (o Leaflet não detectava erro nenhum). `maxNativeZoom: 10` no tile layer: acima disso o Leaflet amplia o tile de zoom 10 em vez de pedir um zoom que não existe.
- **Modo off-line reintroduzido, como recurso opt-in só para a máquina do estande do Fishing Show Brazil 2026.** Tinha sido removido no Bloco C por questão de termos de uso do Esri quanto a cache local — decisão revertida conscientemente para esse uso pontual (SETUR ciente do risco). `scripts/fetch-tiles.js` revivido (mesma lógica de antes) para baixar tiles (zooms 5–10, bbox = `BAHIA_BOUNDS.pad(0.28)`) em `assets/tiles/`, gerando `assets/tiles/manifest.json`. `js/app.js` troca o tile ao vivo pelo cache local automaticamente se o manifesto existir no disco — a instalação online nunca tem esse diretório (`.gitignore`), então nunca entra nesse modo. Expira sozinho a partir de **03/08/2026, 00h (horário da Bahia)**: mesmo com os arquivos ainda no disco, o app volta a exigir o tile ao vivo depois dessa data (`OFFLINE_CACHE_EXPIRES` em `js/app.js`). Ver README, "Modo off-line".
- **Fix: tiles locais faltando (404) no meio de zooms que deveriam funcionar.** A primeira versão do `fetch-tiles.js` baixava só o bbox exato, sem margem — mas o Leaflet pré-carrega um anel de tiles além do viewport visível (`keepBuffer`, padrão 2) para paneamento suave, e o zoom fracionário (`zoomSnap: 0.25`) também pode pedir tile extra na borda. Sem margem, esse anel dava 404 mesmo dentro do `maxBounds`, aparecendo como manchas de textura faltando ao navegar. `MARGIN: 3` tiles em cada direção, em todo zoom, resolve — de ~3.100 para ~4.460 tiles (~45 MB → ~65 MB). Reproduzido e confirmado corrigido via `read_network_requests` no navegador (mesmos tiles que davam 404 antes, 200 OK depois).
- **PWA (Mac): site instalável como app.** `manifest.json` + `sw.js` novos. Ícones em `assets/icons/` derivados do logo oficial (`assets/BAHIA_TURISMO.png`), recortando só a marca "Bahia!" com o sorriso (sem o "TURISMO"), centralizada em fundo branco quadrado — inclui variantes `maskable` com margem de segurança para o corte adaptativo do Android/Chrome. Service worker cacheia o app shell inteiro (~5 MB: HTML/CSS/JS, Leaflet, fontes, GeoJSON, ilustrações de espécie) no primeiro carregamento, para abrir instantâneo depois — sem incluir `assets/tiles/` nem os tiles ao vivo do Esri, pela mesma cautela institucional do modo off-line. Verificado no navegador: manifesto válido com os 10 ícones, service worker registrado e ativo, 100/100 arquivos do app shell em cache com conteúdo íntegro, zero tiles indevidamente cacheados pelo service worker.

## Novos POIs em Jequié e no Extremo Sul (Blocos M, N e O)

Novo prompt, independente dos anteriores: dois POIs novos, de 21 para 23 pontos no mapa. Ambos entram como **secundários** (`s12`, `s13`), nunca principais — os atalhos de teclado `0`–`9` (`jumpToMain`) já cobrem exatamente os 10 POIs principais existentes, e um 11º principal quebraria esse mapeamento.

- **`s12` — Barragem da Pedra, Jequié.** Fecha a lacuna da Zona Turística **Caminhos do Sudoeste**, que não tinha nenhum POI. Reservatório da UHE Pedra (CHESF), no rio de Contas, 18 km a montante de Jequié. Entram só `traira` e `tilapia` — decisão deliberada de restringir a espécies documentalmente defensáveis para a Bacia do Leste. **O rio de Contas é Bacia do Leste, como o Paraguaçu**, e espécies do São Francisco (dourado-do-rio, surubim, tucunaré, piau, pacu) foram explicitamente excluídas, pelo mesmo critério que levou à remoção do antigo POI do Alto Paraguaçu (preservado em bloco comentado ao final de `js/data.js`). Comentários `VALIDAR` registram o caminho para ampliar a lista de espécies (Estação de Piscicultura de Jequié, Bahia Pesca) e a ausência de operadora confirmada.
- **`s13` — Estuários do Extremo Sul.** Amplia a Zona Turística **Costa das Baleias**, que só tinha o `s7` (Caravelas). Reúne o sistema estuarino do rio Jucuruçu (Itamaraju–Prado), do rio Itanhém (Alcobaça) e da enseada de Guaratiba, com robalo abundante em Guaratiba como âncora factual. Entram só `camurim` e `camurim-pena`; camurupim, xaréu, carapeba e pescada-amarela ficam de fora — plausíveis por analogia com Canavieiras e Mangue Seco, mas não confirmados para estes rios. **Pendência bloqueante registrada em comentário:** o entorno concentra RESEX Marinha do Corumbau, PARNA do Monte Pascoal e Terra Indígena Pataxó; falta confirmar com ICMBio/INEMA quais unidades incidem sobre os pontos descritos antes de detalhar `rules` ou levar o POI a material impresso — mesmo aprendizado da RESEX de Canavieiras.
- **Contador de pontos corrigido nos dois lugares onde estava hardcoded** (`index.html` e `renderMatchLabel()` em `js/app.js`), de "21 pontos mapeados" para "23 pontos mapeados". O valor não era calculado a partir de `POIS.length` — os dois literais precisam ser mantidos em sincronia manualmente enquanto não forem unificados.
- **Zonas Turísticas ainda sem nenhum POI, após este prompt:** Caminhos do Sertão e Caminhos do Jiquiriçá (de três lacunas, restam duas).
- **`cherne` continua sendo o único chip morto** (`check-data.js`, item 1) — herança de rodada anterior, não afetada por este prompt; decisão de removê-lo do catálogo ou associá-lo a um POI fica em aberto.
