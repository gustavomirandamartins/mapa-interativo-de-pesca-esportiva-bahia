...
- **Modo off-line reintroduzido, como recurso opt-in só para a máquina do estande do Fishing Show Brazil 2026.** Tinha sido removido no Bloco C por questão de termos de uso do Esri quanto a cache local — decisão revertida conscientemente para esse uso pontual (SETUR ciente do risco). `scripts/fetch-tiles.js` revivido (mesma lógica de antes) para baixar ~3.100 tiles (zooms 5–10, bbox = `BAHIA_BOUNDS.pad(0.28)`) em `assets/tiles/` (~45 MB), gerando `assets/tiles/manifest.json`. `js/app.js` troca o tile ao vivo pelo cache local automaticamente se o manifesto existir no disco — a instalação online nunca tem esse diretório (`.gitignore`), então nunca entra nesse modo. Expira sozinho a partir de **03/08/2026, 00h (horário da Bahia)**: mesmo com os arquivos ainda no disco, o app volta a exigir o tile ao vivo depois dessa data (`OFFLINE_CACHE_EXPIRES` em `js/app.js`). Ver README, "Modo off-line".
